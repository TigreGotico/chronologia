...
| Civil holidays | [civil holidays](docs/civil-holidays.md) — an optional `chronologia[holidays]` bridge onto the `vacanza/holidays` database (the tzdb pattern applied to observances): rules stay in chronologia, decrees are borrowed and handed back as typed `Holiday`/`DateSpan` objects, with estimated lunar dates mapped honestly to `basis="predicted"` |
