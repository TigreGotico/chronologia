# -*- coding: utf-8 -*-
"""Century and decade vocabulary -- "sieglo" (Wiktionary: Aragonese
masculine noun "century", plural "sieglos", a semi-learned borrowing from
Latin saeculum) and "decada"/"decenio" (Wiktionary: Aragonese feminine
noun "decade (10 years)", synonym of "decenio").

Roman-numeral centuries ("sieglo XX") are NOT exercised here: Aragonese is
not in the small set of locales the Roman-numeral fold
(``numfold_roman.py``) covers, so that surface genuinely does not resolve
regardless of this vocabulary addition -- out of scope for this corpus. The
dotted-ordinal digit form ("20. sieglo") works instead, since an's
tokenizer already reads ordinal_dot notation.
...
The relative-offset form ("N sieglo/s fa" = N centuries ago, "fa" being
this locale's citable past marker) resolves through the shared
...
@pytest.mark.parametrize("text,n", [
    ("3. sieglo", 3), ("5. sieglo", 5), ("20. sieglo", 20),
])
...
@pytest.mark.parametrize("text,years", [
    ("un sieglo fa", 100), ("dos sieglos fa", 200),
])
...
@pytest.mark.parametrize("text,decades", [
    ("una decada fa", 1), ("tres decadas fa", 3),
])
...
@pytest.mark.parametrize("text", ["sieglo", "decada", "decenio"])
