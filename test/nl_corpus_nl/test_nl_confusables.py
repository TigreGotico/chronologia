# -*- coding: utf-8 -*-
"""Confusables corpus for nl -- temporal-looking tokens that must NOT bind.
...
_LIMITATIONS = [
    ...
    pytest.param(
        'augustus als voornaam',
        marks=pytest.mark.xfail(reason='month homograph used as a person/pet name binds as the month; expected limitation, disambiguation is downstream', strict=True)),
    pytest.param(
        'de lente der volkeren',
...
_SPAN_ELSEWHERE = [
    ('een seconde stilte over 3 dagen', 'seconde'),
    ('de halve waarheid over 2 weken', 'waarheid'),
]
