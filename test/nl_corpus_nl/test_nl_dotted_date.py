"""The dotted civil date in Dutch: "15.06.2020".

Dutch everyday usage writes "15.06.2020", day first, and that surface has
...
def test_dotted_date_nl():
    assert start_end("15.06.2020") == (AstroDate(2020, 6, 15),
                                       AstroDate(2020, 6, 16))


def test_dotted_date_in_a_sentence_nl():
    s, e = start_end('de vergadering op 15.06.2020')
    assert (s, e) == (AstroDate(2020, 6, 15), AstroDate(2020, 6, 16))
    assert parse('de vergadering op 15.06.2020')[1] == 'de vergadering op'


def test_dotted_date_unpadded_nl():
    """The leading zeros are optional; both forms read alike."""
    assert start_end('15.6.2020') == (AstroDate(2020, 6, 15),
                                      AstroDate(2020, 6, 16))


def test_dotted_date_day_over_twelve_nl():
...
def test_dotted_date_two_digit_year_nl():
...
@pytest.mark.parametrize("text,y,m,d", [('3 januari 2020', 2020, 1, 3)])
def test_written_date_still_reads_nl(text, y, m, d):
    assert start_end(text) == (AstroDate(y, m, d), AstroDate(y, m, d + 1))


def test_a_thousands_group_is_not_a_date_nl():
...
def test_malformed_dotted_run_refuses_nl(text):
    nomatch(text)
