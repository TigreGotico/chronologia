# -*- coding: utf-8 -*-
"""Calendar quarters (uk). Quarter N spans months [3N-2 .. 3N]; anchor
2017-06-27 sits in Q2. Digit and Q-forms fold to the number; ordinal-WORD
forms ("третій квартал") are unsupported because the uk number model does not fold
...
_CASES = [
    ...
    ('наступний квартал', 2017, 7, 2017, 10),
    ('цей квартал', 2017, 4, 2017, 7),
...
@pytest.mark.parametrize("text", ['Q5 2026', 'зустріч'])
...
@pytest.mark.xfail(reason="ordinal-word quarter unsupported: uk number model "
                          "does not fold ordinal words to a numeric ORD",
                   strict=True)
def test_ordinal_word_quarter():
    s, e = start_end("третій квартал 2026")
