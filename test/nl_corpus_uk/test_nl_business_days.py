# -*- coding: utf-8 -*-
"""Business-day counting (uk), holiday-blind (no jurisdiction): a business day
...
def start(text):
    r = extract_timespan(text, "uk", ANCHOR)
...
def test_count(n, expected):
    assert start(f"{n} робочих днів") == _ad(expected)


def test_next_business_day():
    assert start("наступний робочий день") == _ad(date(2017, 6, 28))


def test_span_is_day_wide():
    r = extract_timespan("3 робочих днів", "uk", ANCHOR)
    assert r[0].width == timedelta(days=1)


@pytest.mark.parametrize("text", ["зустріч", "робочих"])
def test_negatives(text):
    assert extract_timespan(text, "uk", ANCHOR) is None
