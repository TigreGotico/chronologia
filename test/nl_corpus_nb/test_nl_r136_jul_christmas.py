"""R136 regression (nb): bare "jul" is Christmas, not the month of July.

Norwegian Bokmal spells the month "juli"; "jul" alone is the everyday word
...
def test_recurrence_until_jul_is_christmas():
    got = extract_recurrence('hver mandag til jul', 'nb', anchor=ANCHOR)
    assert got is not None
    assert got[0].until == AstroDate(2017, 12, 25)


def test_recurrence_until_december_unchanged():
    got = extract_recurrence('hver mandag til desember', 'nb',
                              anchor=ANCHOR)
    assert got is not None
