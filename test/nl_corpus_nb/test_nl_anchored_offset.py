"""Anchored arithmetic (nb): a signed unit offset or a strict weekday roll
...
@pytest.mark.parametrize("text,expected", [
    ("2 uker etter påske", EASTER + timedelta(days=14)),
    ("3 dager etter juledag", XMAS + timedelta(days=3)),
    ("3 dager før påske", EASTER - timedelta(days=3)),
])
def test_unit_offset(text, expected):
    assert start(text) == _ad(expected)

@pytest.mark.parametrize("text,expected", [
    ("mandag etter juledag", date(2018, 1, 1)),
...
def test_week_offset_is_week_wide():
    assert span("2 uker etter påske").width == timedelta(days=7)

def test_bare_after_holiday_unchanged():
    assert start("etter påske") == _ad(EASTER)
