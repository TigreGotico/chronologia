# -*- coding: utf-8 -*-
"""nb: "<ordinal> <weekday> i <month> <year>" -- the Nth weekday of a month,
and "siste <weekday> i <month> <year>" -- the last weekday of a month.

Bokmål scopes the ordinal/last weekday against the named month with the native
connector "i" ("første mandag i januar 2020").  The gold day is computed by
independent calendar arithmetic -- find the first matching weekday of the
month, then step whole weeks (or step back from the month end for the last) --
and is never read back from the parser.  n is kept to 1..4 so the target
weekday always exists in every month.

A BARE "siste <weekday>" with NO trailing month stays anchor-relative
(weekday_ref, last occurrence before the anchor); only the month-scoped form
nests into scoped_ordinal.
"""
...
_WD = {"mandag": 0, "onsdag": 2, "fredag": 4, "søndag": 6}
_MONTHS = {1: "januar", 3: "mars", 5: "mai", 11: "november"}
_ORD = [("første", 1), ("andre", 2), ("annen", 2), ("tredje", 3),
        ("fjerde", 4)]
...
def test_bare_last_weekday_is_anchor_relative():
    # No trailing month -> weekday_ref, the last Friday BEFORE the Tuesday
    # anchor (2017-06-27), i.e. 2017-06-23 -- NOT a month-scoped reading.
