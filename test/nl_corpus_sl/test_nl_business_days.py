# -*- coding: utf-8 -*-
"""Business-day counting (sl), holiday-blind (no jurisdiction): a business day
...
def start(text):
    r = extract_timespan(text, "sl", ANCHOR)
...
def test_count(n, expected):
    assert start(f"{n} delovnih dni") == _ad(expected)


def test_next_business_day():
    assert start("naslednji delovni dan") == _ad(date(2017, 6, 28))


def test_span_is_day_wide():
    r = extract_timespan("3 delovnih dni", "sl", ANCHOR)
    assert r[0].width == timedelta(days=1)


@pytest.mark.parametrize("text", ["sestanek", "delovnih"])
def test_negatives(text):
    assert extract_timespan(text, "sl", ANCHOR) is None
