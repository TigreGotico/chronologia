"""Shared helpers for the Romanian natural-language corpus.

The contract under test is the public span-native edge
``extract_timespan(text, "ro", anchor)``: feed a sentence a Romanian speaker
...
def parse(text, anchor=ANCHOR):
    return extract_timespan(text, "ro", anchor)
