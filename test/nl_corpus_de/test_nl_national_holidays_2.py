# -*- coding: utf-8 -*-
"""German national / civil holidays bound by name (round 2, ``holiday_ref``).
...
_BARE = [
    ('tag der arbeit', (2018, 5, 1)),
    ('tag der deutschen einheit', (2017, 10, 3)),
    ('tag der einheit', (2017, 10, 3)),
    ('nikolaus', (2017, 12, 6)),
    ('nikolaustag', (2017, 12, 6)),
    ('sankt nikolaus', (2017, 12, 6)),
]
...
@pytest.mark.parametrize("text,ymd", [
    ('tag der deutschen einheit 2019', (2019, 10, 3)),
    ('nikolaus 2020', (2020, 12, 6)),
])
...
def test_bare_month_unchanged():
    assert start('dezember') == AstroDate(2017, 12, 1)
    assert span('dezember').width == timedelta(days=31)


def test_bare_weekday_unchanged():
    assert start('montag') == AstroDate(2017, 7, 3)
    assert span('montag').width == timedelta(days=1)
