"""Character offsets (de): every TimeMention carries a char_span into the
...
_SINGLE = ['Q3 2026', 'drittes quartal 2026', 'erstes quartal 2020', 'viertes quartal 2019', 'woche 32', 'kw 32', 'woche 1', 'anfang des monats', 'mitte des monats', 'ende des monats']

@pytest.mark.parametrize("utterance", _SINGLE)
def test_char_span_recovers_full_mention(utterance):
    ms = extract_timespans(utterance, "de", anchor=ANCHOR)
...
def test_char_span_embedded_offset():
    u = "termin 2026-07-05"
    ms = extract_timespans(u, "de", anchor=ANCHOR)
    assert len(ms) == 1
    cs, ce = ms[0].char_span
...
def test_no_mention_empty():
    assert extract_timespans("xxxxx yyyyy zzzzz", "de", anchor=ANCHOR) == []
