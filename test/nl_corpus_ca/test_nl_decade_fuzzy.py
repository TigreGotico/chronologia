# -*- coding: utf-8 -*-
"""early/mid/late composed with the Catalan decade construction (ca):
"a principis/a mitjans/a finals dels anys NNNN" and the word-form decade
"els anys noranta" -- same snap-to-year thirds convention as
...
_CASES = [
    ('a principis dels anys 2000', DECADE_2000S, 'early'),
    ('a mitjans dels anys 2000', DECADE_2000S, 'mid'),
    ('a finals dels anys 2000', DECADE_2000S, 'late'),
    ('a principis dels anys noranta', DECADE_90S, 'early'),
    ('a mitjans dels anys noranta', DECADE_90S, 'mid'),
    ('a finals dels anys noranta', DECADE_90S, 'late'),
]
...
def test_decade_fuzzy(text, decade, part):
    ...
    assert s == want_s
    assert e == want_e
    # "a" is only ca's marker_at surface, not marker_article -- it precedes
    # the match unconsumed, same leftover shape as the Spanish sibling.
    assert parse(text)[1] == "a"
