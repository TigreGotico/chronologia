# -*- coding: utf-8 -*-
"""Business-day counting (bg), holiday-blind (no jurisdiction): a business day
...
def start(text):
    r = extract_timespan(text, "bg", ANCHOR)
...
def test_count(n, expected):
    assert start(f"{n} работни дни") == _ad(expected)


def test_next_business_day():
    assert start("следващ работен ден") == _ad(date(2017, 6, 28))


def test_span_is_day_wide():
    r = extract_timespan("3 работни дни", "bg", ANCHOR)
    assert r[0].width == timedelta(days=1)


@pytest.mark.parametrize("text", ["срещата", "работни"])
def test_negatives(text):
    assert extract_timespan(text, "bg", ANCHOR) is None
