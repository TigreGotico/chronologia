...
staples exactly. Pairs are read as a literal from test_nl_dates_ranges_adversarial.py so this
file stays import-free.
"""
import ast as _ast
import os as _os

_src = open(_os.path.join(_os.path.dirname(__file__), "test_nl_dates_ranges_adversarial.py"),
            encoding="utf-8").read()
PARITY = next(
