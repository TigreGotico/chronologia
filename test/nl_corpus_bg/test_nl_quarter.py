# -*- coding: utf-8 -*-
"""Calendar quarters (bg). Quarter N spans months [3N-2 .. 3N]; anchor
2017-06-27 sits in Q2. Digit and Q-forms fold to the number; ordinal-WORD
forms ("трето тримесечие") are unsupported because the bg number model does not fold
...
_CASES = [
    ...
    ('3 тримесечие 2026', 2026, 7, 2026, 10),
    ('1 тримесечие 2020', 2020, 1, 2020, 4),
    ('2 тримесечие 2018', 2018, 4, 2018, 7),
    ('4 тримесечие 2019', 2019, 10, 2020, 1),
    ('следващо тримесечие', 2017, 7, 2017, 10),
    ('това тримесечие', 2017, 4, 2017, 7),
...
@pytest.mark.parametrize("text", ['Q5 2026', 'срещата'])
...
@pytest.mark.xfail(reason="ordinal-word quarter unsupported: bg number model "
                          "does not fold ordinal words to a numeric ORD",
                   strict=True)
def test_ordinal_word_quarter():
    s, e = start_end("трето тримесечие 2026")
