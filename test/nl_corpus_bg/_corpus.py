"""Shared helpers for the Bulgarian natural-language corpus.

The contract under test is the public span-native edge
``extract_timespan(text, "bg", anchor)``: feed a sentence a Bulgarian speaker
...
Bulgarian is a heavily inflected South-Slavic language: the offset unit noun
...
def parse(text, anchor=ANCHOR):
    return extract_timespan(text, "bg", anchor)
