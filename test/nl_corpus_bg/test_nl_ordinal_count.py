# -*- coding: utf-8 -*-
"""Ordinal counting from the anchor (bg). "weekend after next" = skip one
...
The "N weekday from now" form is unnatural in bg (no colloquial "from now"
...
def test_weekend_after_next():
    s = span("уикенд след следващия")
    assert (s.start.year, s.start.month, s.start.day) == (2017, 7, 15)
    assert s.width == timedelta(days=2)


@pytest.mark.xfail(reason="no colloquial 'from now' present-marker in bg; the "
                          "N-weekday-from-now count does not fire",
                   strict=True)
def test_weekday_from_now():
    # would be the 3rd понеделник strictly after the anchor
    assert start("3 понеделник след следващия") == AstroDate(2017, 7, 17)
