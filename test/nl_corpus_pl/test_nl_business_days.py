# -*- coding: utf-8 -*-
"""Business-day counting (pl), holiday-blind (no jurisdiction): a business day
...
def start(text):
    r = extract_timespan(text, "pl", ANCHOR)
...
def test_count(n, expected):
    assert start(f"{n} roboczych dni") == _ad(expected)


def test_next_business_day():
    assert start("następny roboczy dzień") == _ad(date(2017, 6, 28))


def test_span_is_day_wide():
    r = extract_timespan("3 roboczych dni", "pl", ANCHOR)
    assert r[0].width == timedelta(days=1)


@pytest.mark.parametrize("text", ["spotkanie", "roboczych"])
def test_negatives(text):
    assert extract_timespan(text, "pl", ANCHOR) is None
