...
Portuguese frames a decade with the plural "os anos 80"; the
singular "o ano 1980" is the ordinary way to write a year.  Golds are computed here by independent arithmetic: a year N spans
...
def test_early_part_of_a_decade():
    assert resolved("no início dos anos 80") == (ad(datetime(1980, 1, 1)),
                                                 ad(datetime(1983, 1, 1)))


def test_late_part_of_a_decade():
    assert resolved("no fim dos anos 80") == (ad(datetime(1987, 1, 1)),
                                              ad(datetime(1990, 1, 1)))


def test_year_introduced_by_de():
    assert resolved("no ano de 1980") == year(1980)
...
def test_dated_day():
    assert resolved("15 de março de 2026") == (ad(datetime(2026, 3, 15)),
