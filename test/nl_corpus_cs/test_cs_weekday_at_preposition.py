...
"v neděli" names the same Sunday as the bare weekday alone; "v" is not
...
def test_bare_nedeli_is_sunday_cs():
    s = span("neděli")
    assert (s.end - s.start) == timedelta(days=1)
    assert s.start.date().weekday() == 6


def test_v_nedeli_consumes_the_preposition_cs():
    r = parse("v neděli")
    assert r is not None
    assert r[0].start.date().weekday() == 6
    assert r[0] == span("neděli", ANCHOR)
    assert r[1] == ""
