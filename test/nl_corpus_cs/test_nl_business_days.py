# -*- coding: utf-8 -*-
"""Business-day counting (cs), holiday-blind (no jurisdiction): a business day
...
def start(text):
    r = extract_timespan(text, "cs", ANCHOR)
...
def test_count(n, expected):
    assert start(f"{n} pracovních dní") == _ad(expected)


def test_next_business_day():
    assert start("příští pracovní den") == _ad(date(2017, 6, 28))


def test_span_is_day_wide():
    r = extract_timespan("3 pracovních dní", "cs", ANCHOR)
    assert r[0].width == timedelta(days=1)


@pytest.mark.parametrize("text", ["schůzku", "pracovních"])
def test_negatives(text):
    assert extract_timespan(text, "cs", ANCHOR) is None
