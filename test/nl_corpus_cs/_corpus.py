"""Shared helpers for the Czech natural-language corpus.

The contract under test is the public span-native edge
``extract_timespan(text, "cs", anchor)``: feed a sentence a Czech speaker
...
Czech is a heavily inflected West-Slavic language: the offset unit noun
...
def parse(text, anchor=ANCHOR):
    return extract_timespan(text, "cs", anchor)
