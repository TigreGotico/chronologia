# -*- coding: utf-8 -*-
"""Multi-mention in he: extract_timespans -> all mentions in order."""
import pytest

from chronologia.extract import extract_timespans
from ._corpus import ANCHOR

LANG = "he"


def mentions(text):
    return extract_timespans(text, LANG, ANCHOR)


@pytest.mark.parametrize("text,count", [
    ('20 ביוני 2026 או 4 באוגוסט 2026', 2),
    ('אתמול היום מחר', 3),
    ('מחר', 1),
    ('אין כאן שום דבר זמני', 0),
])
def test_mention_count(text, count):
    assert len(mentions(text)) == count


def test_two_mentions_in_order():
    ms = mentions('20 ביוני 2026 או 4 באוגוסט 2026')
    assert [m.text for m in ms] == ['20 ביוני 2026', '4 באוגוסט 2026']


def test_three_named_days_present():
    ms = mentions('אתמול היום מחר')
    assert len(ms) == 3
