"""Spanish does not write the dotted date, so it reads as nothing.
...
Spanish writes the numeric date with slashes, so the dotted surface is not a
Spanish date and must not be read as one.
...
def test_dotted_date_refused_es(text):
    nomatch(text)


def test_the_slashed_date_still_reads_es():
    """The numeric date Spanish does write is untouched."""
    assert start_end("15/06/2020") == (AstroDate(2020, 6, 15),
                                       AstroDate(2020, 6, 16))


def test_year_first_dots_still_read_es():
    """A four-digit lead is year-first in every language, dots included."""
    assert start_end("2020.06.15") == (AstroDate(2020, 6, 15),
                                       AstroDate(2020, 6, 16))


def test_the_written_year_range_survives_es():
