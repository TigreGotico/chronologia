...
Spanish frames a decade with the plural "los años 1980"; the singular "el
año 1980" names the single year 1980.  Golds are computed here by
...
def test_singular_year_word_is_one_year(n):
    assert resolved(f"el año {n}") == year(n)


@pytest.mark.parametrize("n", [1920, 1980, 2000])
def test_plural_year_word_is_a_decade(n):
    assert resolved(f"los años {n}") == decade(n)


def test_plural_year_word_off_the_ten_is_one_year():
    assert resolved("los años 1914") == year(1914)


# -- controls: surfaces that must keep working --------------------------

def test_bare_tens_decade():
    assert resolved("los años 80") == decade(1980)


def test_spelled_tens_decade():
    assert resolved("los años noventa") == decade(1990)


def test_early_part_of_a_decade():
    assert resolved("principios de los años 80") == (ad(datetime(1980, 1, 1)),
...
def test_year_range():
    assert resolved("entre 2010 y 2020") == (ad(datetime(2010, 1, 1)),
                                             ad(datetime(2021, 1, 1)))


def test_decade_bc_unchanged():
    s, e = resolved("los años 80 a.C.")
