# -*- coding: utf-8 -*-
"""Swedish national / civil holidays bound by name (round 2, ``holiday_ref``).
...
_BARE = [
    ('sveriges nationaldag', (2018, 6, 6)),
    ('nationaldagen', (2018, 6, 6)),
    ('svenska flaggans dag', (2018, 6, 6)),
    ('trettondedag jul', (2018, 1, 6)),
]
...
@pytest.mark.parametrize("text,ymd", [
    ('sveriges nationaldag 2019', (2019, 6, 6)),
])
...
def test_bare_month_unchanged():
    assert start('juni') == AstroDate(2017, 6, 1)
    assert span('juni').width == timedelta(days=30)


def test_bare_weekday_unchanged():
    assert start('måndag') == AstroDate(2017, 7, 3)
    assert span('måndag').width == timedelta(days=1)
