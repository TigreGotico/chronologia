# -*- coding: utf-8 -*-
"""Calendar quarters (sk). Quarter N spans months [3N-2 .. 3N]; anchor
2017-06-27 sits in Q2. Digit and Q-forms fold to the number; ordinal-WORD
forms ("tretí kvartál") are unsupported because the sk number model does not fold
...
@pytest.mark.parametrize("text", ['Q5 2026', 'stretnutie'])
...
@pytest.mark.xfail(reason="ordinal-word quarter unsupported: sk number model "
                          "does not fold ordinal words to a numeric ORD",
                   strict=True)
def test_ordinal_word_quarter():
    s, e = start_end("tretí kvartál 2026")
