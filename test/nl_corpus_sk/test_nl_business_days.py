# -*- coding: utf-8 -*-
"""Business-day counting (sk), holiday-blind (no jurisdiction): a business day
...
def start(text):
    r = extract_timespan(text, "sk", ANCHOR)
...
def test_count(n, expected):
    assert start(f"{n} pracovných dní") == _ad(expected)


def test_next_business_day():
    assert start("budúci pracovný deň") == _ad(date(2017, 6, 28))


def test_span_is_day_wide():
    r = extract_timespan("3 pracovných dní", "sk", ANCHOR)
    assert r[0].width == timedelta(days=1)


@pytest.mark.parametrize("text", ["stretnutie", "pracovných"])
def test_negatives(text):
    assert extract_timespan(text, "sk", ANCHOR) is None
