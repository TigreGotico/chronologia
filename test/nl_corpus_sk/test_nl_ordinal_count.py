# -*- coding: utf-8 -*-
"""Ordinal counting from the anchor (sk). "weekend after next" = skip one
...
The "N weekday from now" form is unnatural in sk (no colloquial "from now"
...
def test_weekend_after_next():
    s = span("víkend po budúcom")
    assert (s.start.year, s.start.month, s.start.day) == (2017, 7, 15)
    assert s.width == timedelta(days=2)


@pytest.mark.xfail(reason="no colloquial 'from now' present-marker in sk; the "
                          "N-weekday-from-now count does not fire",
                   strict=True)
def test_weekday_from_now():
    # would be the 3rd pondelok strictly after the anchor
    assert start("3 pondelok po budúcom") == AstroDate(2017, 7, 17)
