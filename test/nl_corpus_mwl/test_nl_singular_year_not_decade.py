...
Mirandese frames a decade with the plural "ls anhos 80"; the
singular "l anho 1980" names the single year 1980.  Golds are computed here by independent arithmetic: a year N spans
...
def test_bare_tens_decade():
    assert resolved("ls anhos 80") == decade(1980)


def test_spelled_tens_decade():
    assert resolved("ls anhos nobenta") == decade(1990)
...
def test_dated_day():
    assert resolved("15 de márcio de 2026") == (ad(datetime(2026, 3, 15)),
