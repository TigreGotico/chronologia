...
Root cause (see chronologia/extract/matcher.py, HOUR slot): ``da`` is an
...
"10.42" HH.MM decimal, so ``clock_time`` never matched and "fredag klokken
10." stayed a date-only span with "klokken 10." stranded in the remainder --
while plain "fredag klokken 10" (no trailing period) merged correctly.
"""
...
def test_sentence_final_period_merges():
    assert start("Mødet er fredag klokken 10.") == ad(_FRIDAY_10)
...
def test_bare_marker_hour_period():
    assert start("klokken 10.") == ad(_NEXT_10)
    assert start("klokken 10") == ad(_NEXT_10)
