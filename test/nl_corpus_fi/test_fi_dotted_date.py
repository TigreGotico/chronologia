"""The dotted civil date in Finnish: "15.06.2020".

SFS 4175 writes the Finnish date without leading zeros, "15.6.2020", day
first, and that surface has to read as the day it names.  Reading only the
year out of it and stranding the day and the month in the remainder is a
silent wrong: the caller gets a confident whole-year span with nothing to
tell it the day was lost.
...
def test_dotted_date_fi():
    assert start_end("15.06.2020") == (AstroDate(2020, 6, 15),
                                       AstroDate(2020, 6, 16))


def test_dotted_date_in_a_sentence_fi():
    s, e = start_end('tapaaminen 15.6.2020')
    assert (s, e) == (AstroDate(2020, 6, 15), AstroDate(2020, 6, 16))
    assert parse('tapaaminen 15.6.2020')[1] == 'tapaaminen'


def test_dotted_date_unpadded_fi():
    """The leading zeros are optional; both forms read alike."""
    assert start_end('15.06.2020') == (AstroDate(2020, 6, 15),
                                       AstroDate(2020, 6, 16))


def test_dotted_date_day_over_twelve_fi():
...
def test_dotted_date_two_digit_year_fi():
...
@pytest.mark.parametrize("text,y,m,d", [('3. tammikuuta 2020', 2020, 1, 3)])
def test_written_date_still_reads_fi(text, y, m, d):
    assert start_end(text) == (AstroDate(y, m, d), AstroDate(y, m, d + 1))


def test_a_thousands_group_is_not_a_date_fi():
...
def test_malformed_dotted_run_refuses_fi(text):
    nomatch(text)
...
def test_a_broken_date_is_not_a_bare_year_fi(text):
...
def test_trailing_sentence_dot_fi():
