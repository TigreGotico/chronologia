# -*- coding: utf-8 -*-
"""fi month-thirds with an explicit TRAILING year: "early/mid/late <month>
...
def test_early_month_third_binds_explicit_year():
    assert start_end("maaliskuun alku 2019") == _G["early"]


def test_mid_month_third_binds_explicit_year():
    assert start_end("maaliskuun puoliväli 2019") == _G["mid"]


def test_late_month_third_binds_explicit_year():
    assert start_end("maaliskuun loppu 2019") == _G["late"]
...
def test_yearless_month_third_stays_anchor_year_regression():
    ...
    assert start_end("maaliskuun alku") == early
