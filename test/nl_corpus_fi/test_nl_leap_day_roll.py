...
leap year, so ``29. helmikuuta`` is 2020-02-29 -- the next February that has a
29th.  ``28. helmikuuta`` keeps the ordinary roll to 2018-02-28.
...
def test_bare_feb_28_keeps_the_ordinary_roll():
    r = parse("28. helmikuuta")
    assert r is not None
    assert r.span.start == ad(datetime(2018, 2, 28))
    assert r.remainder == ""


def test_impossible_day_number_is_not_a_date():
    nomatch("45. tammikuuta")

def test_named_leap_year_resolves_in_that_year():
    r = parse("29. helmikuuta 2020")
    assert r is not None
    assert r.span.start == ad(datetime(2020, 2, 29))
    assert r.remainder == ""
