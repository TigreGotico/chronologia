# -*- coding: utf-8 -*-
"""Calendar quarters (hr). Quarter N spans months [3N-2 .. 3N]; anchor
2017-06-27 sits in Q2. Digit and Q-forms fold to the number; ordinal-WORD
forms ("treći kvartal") are unsupported because the hr number model does not fold
...
_CASES = [
    ...
    ('4 kvartal 2019', 2019, 10, 2020, 1),
    ('sljedeći kvartal', 2017, 7, 2017, 10),
    ('ovaj kvartal', 2017, 4, 2017, 7),
...
@pytest.mark.parametrize("text", ['Q5 2026', 'sastanak'])
...
@pytest.mark.xfail(reason="ordinal-word quarter unsupported: hr number model "
                          "does not fold ordinal words to a numeric ORD",
                   strict=True)
def test_ordinal_word_quarter():
    s, e = start_end("treći kvartal 2026")
