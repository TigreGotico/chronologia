# -*- coding: utf-8 -*-
"""Ordinal counting from the anchor (hr). "weekend after next" = skip one
...
The "N weekday from now" form is unnatural in hr (no colloquial "from now"
...
def test_weekend_after_next():
    s = span("vikend nakon sljedećeg")
    assert (s.start.year, s.start.month, s.start.day) == (2017, 7, 15)
    assert s.width == timedelta(days=2)


@pytest.mark.xfail(reason="no colloquial 'from now' present-marker in hr; the "
                          "N-weekday-from-now count does not fire",
                   strict=True)
def test_weekday_from_now():
    # would be the 3rd ponedjeljak strictly after the anchor
    assert start("3 ponedjeljak nakon sljedećeg") == AstroDate(2017, 7, 17)
