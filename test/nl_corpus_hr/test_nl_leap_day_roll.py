...
leap year, so ``29. veljače`` is 2020-02-29 -- the next February that has a
29th.  ``28. veljače`` keeps the ordinary roll to 2018-02-28.
...
def test_impossible_day_number_is_not_a_date():
    nomatch("30 veljača 2020")

def test_named_leap_year_resolves_in_that_year():
    r = parse("29. veljače 2020")
    assert r is not None
    assert r.span.start == ad(datetime(2020, 2, 29))
    assert r.remainder == ""
