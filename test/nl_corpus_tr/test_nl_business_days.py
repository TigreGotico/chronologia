"""Business-day counting (tr): "N işgünü sonra". Holiday-blind default: Mon-Fri
only, strictly after Tue 2017-06-27. Grid: Wed28(1) Thu29(2) Fri30(3) Mon
Jul3(4) Tue4(5) Wed5(6). The two-word "iş günü" is not a single-token business
marker, so the closed compound "işgünü" is used."""
...
def test_business_day_is_day_wide():
    r = extract_timespan("3 işgünü sonra", "tr", A)
    assert r[0].width == timedelta(days=1)

@pytest.mark.parametrize("text", ["işgünü", "yoğun işgünü", "işe dönüş"])
def test_negatives(text):
    assert extract_timespan(text, "tr", A) is None
