"""The dotted civil date in Turkish: "15.06.2020".

the TDK writing rules give the Turkish date as "15.06.2020", day first,
and that surface has to read as the day it names.  Reading only the year
out of it and stranding the day and the month in the remainder is a silent
wrong: the caller gets a confident whole-year span with nothing to tell it
the day was lost.
...
def test_dotted_date_tr():
    assert start_end("15.06.2020") == (AstroDate(2020, 6, 15),
                                       AstroDate(2020, 6, 16))


def test_dotted_date_in_a_sentence_tr():
    s, e = start_end('toplantı 15.06.2020')
    assert (s, e) == (AstroDate(2020, 6, 15), AstroDate(2020, 6, 16))
    assert parse('toplantı 15.06.2020')[1] == 'toplantı'


def test_dotted_date_unpadded_tr():
    """The leading zeros are optional; both forms read alike."""
    assert start_end('15.6.2020') == (AstroDate(2020, 6, 15),
                                      AstroDate(2020, 6, 16))


def test_dotted_date_day_over_twelve_tr():
...
def test_dotted_date_two_digit_year_tr():
...
@pytest.mark.parametrize("text,y,m,d", [('3 Ocak 2020', 2020, 1, 3)])
def test_written_date_still_reads_tr(text, y, m, d):
    assert start_end(text) == (AstroDate(y, m, d), AstroDate(y, m, d + 1))


def test_a_thousands_group_is_not_a_date_tr():
...
def test_malformed_dotted_run_refuses_tr(text):
    nomatch(text)
...
def test_a_broken_date_is_not_a_bare_year_tr(text):
...
def test_trailing_sentence_dot_tr():
