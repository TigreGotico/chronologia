"""Business-day counting (ms): "N hari kerja". Holiday-blind default: Mon-Fri
...
def test_business_day_is_day_wide():
    r = extract_timespan("3 hari kerja", "ms", A)
    assert r[0].width == timedelta(days=1)

@pytest.mark.parametrize("text", ["hari kerja", "kembali kerja", "waktu kerja"])
def test_negatives(text):
    assert extract_timespan(text, "ms", A) is None
