# -*- coding: utf-8 -*-
"""Business-day counting (ru), holiday-blind (no jurisdiction): a business day
...
def start(text):
    r = extract_timespan(text, "ru", ANCHOR)
...
def test_count(n, expected):
    assert start(f"{n} рабочих дней") == _ad(expected)


def test_next_business_day():
    assert start("следующий рабочий день") == _ad(date(2017, 6, 28))


def test_span_is_day_wide():
    r = extract_timespan("3 рабочих дней", "ru", ANCHOR)
    assert r[0].width == timedelta(days=1)


@pytest.mark.parametrize("text", ["встречу", "рабочих"])
def test_negatives(text):
    assert extract_timespan(text, "ru", ANCHOR) is None
