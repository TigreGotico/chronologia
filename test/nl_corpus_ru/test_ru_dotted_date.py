"""The dotted civil date in Russian: "15.06.2020".

GOST R 6.30-2003 writes the Russian date "15.06.2020", day first, and that
...
def test_dotted_date_ru():
    assert start_end("15.06.2020") == (AstroDate(2020, 6, 15),
                                       AstroDate(2020, 6, 16))


def test_dotted_date_in_a_sentence_ru():
    s, e = start_end('встреча 15.06.2020')
    assert (s, e) == (AstroDate(2020, 6, 15), AstroDate(2020, 6, 16))
    assert parse('встреча 15.06.2020')[1] == 'встреча'


def test_dotted_date_unpadded_ru():
    """The leading zeros are optional; both forms read alike."""
    assert start_end('15.6.2020') == (AstroDate(2020, 6, 15),
                                      AstroDate(2020, 6, 16))


def test_dotted_date_day_over_twelve_ru():
...
def test_dotted_date_two_digit_year_ru():
...
@pytest.mark.parametrize("text,y,m,d", [('3 января 2020', 2020, 1, 3)])
def test_written_date_still_reads_ru(text, y, m, d):
    assert start_end(text) == (AstroDate(y, m, d), AstroDate(y, m, d + 1))


def test_a_thousands_group_is_not_a_date_ru():
...
def test_malformed_dotted_run_refuses_ru(text):
    nomatch(text)
...
def test_a_broken_date_is_not_a_bare_year_ru(text):
...
def test_trailing_sentence_dot_ru():
    """A dot that ends the sentence is punctuation, not a fourth component."""
    assert start_end("15.06.2020.") == (AstroDate(2020, 6, 15),
                                        AstroDate(2020, 6, 16))


def test_dotted_date_with_a_clock_ru():
    """The "в" stitching the clock onto the date is part of the composed
    reading, not leftover text -- the remainder comes back empty."""
    s, e = start_end('15.06.2020 в 14:30')
    assert s == AstroDate(2020, 6, 15, 14, 30)
    assert parse('15.06.2020 в 14:30')[1] == ''
