# -*- coding: utf-8 -*-
"""nn: the spaced day deictics CLDR gives as the standard surfaces.

Unicode CLDR 47 ``cldr-dates-full/main/nn/dateFields.json`` spells the
day deictics of Norwegian Nynorsk with a space -- ``day.relative-type--2`` through
...
@pytest.mark.parametrize("text,off", [('i førgår', -2), ('i går', -1), ('i dag', 0), ('i morgon', 1), ('i overmorgon', 2)])
def test_spaced_day_deictic(text, off):
    _whole_day(text, off)


@pytest.mark.parametrize("text,off", [('iforgårs', -2), ('igår', -1), ('idag', 0), ('imorgon', 1), ('overmorgon', 2)])
def test_written_together_day_deictic_still_binds(text, off):
    _whole_day(text, off)


def test_bare_morgon_is_still_tomorrow():
    """Morgon carries no morning band in Norwegian Nynorsk; bare, it is the day word."""
    s = span("morgon")
