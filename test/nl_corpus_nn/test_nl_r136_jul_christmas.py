"""R136 regression (nn): bare "jul" is Christmas, not the month of July.

Norwegian Nynorsk spells the month "juli"; "jul" alone is the everyday word
for Christmas ("jul", "juledag", "god jul"). The MONTH vocab also lists
...
def test_recurrence_until_jul_is_christmas():
    got = extract_recurrence('kvar måndag til jul', 'nn', anchor=ANCHOR)
    assert got is not None
    assert got[0].until == AstroDate(2017, 12, 25)


def test_recurrence_until_december_unchanged():
    got = extract_recurrence('kvar måndag til desember', 'nn',
                              anchor=ANCHOR)
    assert got is not None
